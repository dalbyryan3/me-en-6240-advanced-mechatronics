...
int main(void) {
  ...
  while (1) {
    NU32_WriteUART3("Press enter to sample A5 (Pin 11 on pic, B5 on the NU32):\r\n");
    NU32_ReadUART3(msg, 100);
    if (msg[0] != 0) // If first charcter is null character then enter was first entered character
    {
      char firstChar = msg[0];
      sprintf(msg, "The first character entered was not enter, it was: %c\r\nTry again\r\n", firstChar);
      NU32_WriteUART3(msg);
      continue;
    }
