function pidtest_with_noise_fo_filter(Kp, Ki, Kd, N)
...
for i=1:NUMSAMPS
  e = r - s(i) + 0.01*max(s)*randn;         
  edot = e - eprev;

  ud_filtered = N * Kd * edot + (ud_filtered_prev * (1-N*DT));
  ud_filtered_prev = ud_filtered;

  eint = eint + e;
  u(i) = Kp*e + Ki*eint + ud_filtered;
  if (u(i) > UMAX)
    u(i) = UMAX;
  elseif (u(i) < -UMAX)
    u(i) = -UMAX;
...
hold on;
plot(t(1:NUMSAMPS),u/20,'--','Color','y');
set(gca,'FontSize',18);
